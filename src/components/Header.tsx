...
const learnPages = [
  { href: '/music-theory-basics', label: 'Music Theory Basics' },
  { href: '/chord-progressions', label: 'Chord Progressions' },
  { href: '/scales-and-modes', label: 'Scales & Modes' },
  { href: '/intervals-explained', label: 'Intervals Explained' },
  { href: '/ear-training', label: 'Ear Training' },
  { href: '/song-structure', label: 'Song Structure' },
  { href: '/songwriting-tips', label: 'Songwriting Tips' },
  { href: '/how-to-write-a-song', label: 'How to Write a Song' },
  { href: '/music-theory-for-songwriters', label: 'Theory for Songwriters' },
  { href: '/best-songwriting-course', label: 'Best Course Review' },
];
    ...
    <header className="sticky top-0 z-50 bg-dark-900/80 backdrop-blur-xl border-b border-white/5">
      <nav className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2.5 group">
          <svg className="w-8 h-8 text-primary group-hover:scale-110 transition-transform" viewBox="0 0 32 32" fill="none">
            <defs>
            ...
            <path d="M12 4v18.5M24 2v16.5" stroke="url(#noteGrad)" strokeWidth="2.5" strokeLinecap="round" />
            <path d="M12 4l12-2" stroke="url(#noteGrad)" strokeWidth="2.5" strokeLinecap="round" />
            <ellipse cx="8.5" cy="23.5" rx="4.5" ry="3.5" fill="url(#noteGrad)" />
            <ellipse cx="20.5" cy="19.5" rx="4.5" ry="3.5" fill="url(#noteGrad)" />
          </svg>
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link href="/go/songwriting" className="bg-accent hover:bg-accent-light text-dark-900 font-bold px-5 py-2 rounded-lg transition-all text-sm glow-accent hover:scale-105">
            Get the Course
          </Link>
        </div>
      ...
      {mobileOpen && (
        <div className="md:hidden glass-card border-t border-white/5 px-4 py-6 space-y-3 animate-fade-in-up">
          ...
          <Link href="/go/songwriting" className="block bg-accent text-dark-900 font-bold px-4 py-3 rounded-lg text-center mt-4 glow-accent" onClick={() => setMobileOpen(false)}>
            Get the Course →
          </Link>
        </div>
      )}
